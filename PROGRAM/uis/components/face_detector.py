...
from PROGRAM import choices, face_detector, state_manager, wording
from PROGRAM.common_helper import calc_float_step, get_last
from PROGRAM.typing import Angle, FaceDetectorModel, Score
from PROGRAM.uis.core import register_ui_component
...
def render() -> None:
	...
	if state_manager.get_item('face_detector_size') in PROGRAM.choices.face_detector_set[state_manager.get_item('face_detector_model')]:
		face_detector_size_dropdown_options['choices'] = PROGRAM.choices.face_detector_set[state_manager.get_item('face_detector_model')]
	with gradio.Row():
		FACE_DETECTOR_MODEL_DROPDOWN = gradio.Dropdown(
			label = wording.get('uis.face_detector_model_dropdown'),
			choices = PROGRAM.choices.face_detector_set.keys(),
	...
	FACE_DETECTOR_ANGLES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = PROGRAM.choices.face_detector_angles,
	...
	FACE_DETECTOR_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(PROGRAM.choices.face_detector_score_range),
		minimum = PROGRAM.choices.face_detector_score_range[0],
		maximum = PROGRAM.choices.face_detector_score_range[-1]
	)
...
def update_face_detector_angles(face_detector_angles : Sequence[Angle]) -> gradio.CheckboxGroup:
	face_detector_angles = face_detector_angles or PROGRAM.choices.face_detector_angles
