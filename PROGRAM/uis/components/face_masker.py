...
from PROGRAM.typing import FaceMaskRegion, FaceMaskType
...
def render() -> None:
	...
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = PROGRAM.choices.face_mask_types,
	...
	FACE_MASK_REGIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = PROGRAM.choices.face_mask_regions,
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calc_float_step(PROGRAM.choices.face_mask_blur_range),
		minimum = PROGRAM.choices.face_mask_blur_range[0],
		maximum = PROGRAM.choices.face_mask_blur_range[-1],
	...
	with gradio.Group():
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calc_int_step(PROGRAM.choices.face_mask_padding_range),
				minimum = PROGRAM.choices.face_mask_padding_range[0],
				maximum = PROGRAM.choices.face_mask_padding_range[-1],
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider]:
	face_mask_types = face_mask_types or PROGRAM.choices.face_mask_types
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or PROGRAM.choices.face_mask_regions
