...
from PROGRAM import face_landmarker, state_manager, wording
from PROGRAM.common_helper import calc_float_step
from PROGRAM.typing import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = PROGRAM.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(PROGRAM.choices.face_landmarker_score_range),
		minimum = PROGRAM.choices.face_landmarker_score_range[0],
		maximum = PROGRAM.choices.face_landmarker_score_range[-1]
	)
